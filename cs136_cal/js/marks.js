angular.module('marksApp', [])
    .controller('MarksController', function () {
        ...
        function show_text(){
            ...
            if (x.style.display === "none" && display_or_not) {
                x.style.display = "block";
            } else if (!display_or_not){
        ...
        marksList.calculate = function () {
            ...
            if (!isNaN(midterm_mark)) display_or_not = true;
            ...
            if (sel_val == "final") { // want to achieve expected grades by higher final marks
                ...
                var leftMark = aim - currentMark;
                var r_asgmt = 20 * marksList.remainingAssignmentN / total;
                var ras_mark = Math.floor(Math.random() * (r_asgmt));
                var left_final = (aim - currentMark - ras_mark) * 2;
                if (left_final > 100){
                    var r_left_final = left_final - 100;
                    ras_mark += r_left_final;
                    left_final = 100;
                }
                marksList.finalAim = left_final;
                marksList.assignmentAim = (ras_mark / marksList.remainingAssignmentN) / (20 / total) * 100;
                marksList.assignmentAim = marksList.assignmentAim > 0 ? marksList.assignmentAim : 0;
